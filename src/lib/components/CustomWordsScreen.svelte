<script lang="ts">
  ...
  import { getCustomWords, saveCustomWords, wordLevel, escapeHtml } from '../utils/helpers';

  let customWords = $state(getCustomWords());
  let wordInput = $state('');
  let defInput = $state('');
  let result = $state<{ type: string; text: string } | null>(null); // { type: 'ok'|'err', text: '' }
  ...
  <div class="card">
    ...
    <div class="cw-form">
      ...
      <div class="cw-list">
        ...
        {:else}
          {#each customWords as cw, i (cw.w)}
            <div class="cw-item">
              <span class="cw-word">{escapeHtml(cw.w)}</span>
              <span class="cw-def"
                >{escapeHtml(cw.d).substring(0, 40)}{cw.d.length > 40 ? '...' : ''}</span
              >
