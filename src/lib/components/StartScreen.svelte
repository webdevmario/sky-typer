<script>
  ...
  import { getHighScores, clearHighScores, escapeHtml } from '../utils/helpers.js';

  let highScores = $state(getHighScores());
  let nameValue = $state('');

  function refreshScores() { highScores = getHighScores(); }
  ...
  [...new Set(getHighScores().map(h => h.n))].slice(0, 5)
);
</script>
...
<div class="overlay" class:visible={game.screen === 'start'}>
  <div class="card">
    ...
    <div class="form">
      <div class="field">
        <div class="label center">Your Name</div>
        <input
          class="input-field"
          type="text"
...
{#if recentPlayers.length > 0}
      <div class="quick-names">
        {#each recentPlayers as name}
          <button
            class="name-chip"
            class:active={nameValue.toUpperCase() === name}
            onclick={() => { nameValue = name; game.setPlayerName(name); }}
          >{name}</button>
        {/each}
      </div>
    {/if}
    ...
    <div class="field">
      <div class="label center">Difficulty</div>
      <div class="toggle-row">
        <button class="toggle-btn" class:selected={game.difficulty === 'easy'} onclick={() => game.setDifficulty('easy')}>🌱 Easy</button>
        <button class="toggle-btn" class:selected={game.difficulty === 'normal'} onclick={() => game.setDifficulty('normal')}>⚡ Normal</button>
        <button class="toggle-btn" class:selected={game.difficulty === 'hard'} onclick={() => game.setDifficulty('hard')}>🔥 Hard</button>
      </div>
    </div>

    <div class="field">
      <div class="label center">Time Per Level</div>
      <div class="time-row">
        <button class="time-btn" onclick={() => game.setTimePerLevel(game.timePerLevel - 5)}>−</button>
    ...
    <div class="scores-section">
      ...
      <div class="secondary-actions">
        <button class="btn btn-ghost btn-sm" onclick={() => game.openCustomWords()}>📝 Custom Words</button>
...
<style>
  ...
  .card::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.2); border-radius: 3px; }
  .overlay.visible .card { transform: scale(1) translateY(0); }

  .emoji-hero { font-size: 48px; margin-bottom: 8px; }
  .title {
    font-family: 'Lilita One', sans-serif; font-size: 34px; margin-bottom: 6px;
  ...
  .form { text-align: left; margin: 0 auto; max-width: 380px; }
  .field { margin-bottom: 16px; }
  .label {
    display: block; font-family: 'Fredoka', sans-serif; font-size: 11px;
  ...
  .input-field {
    ...
    border-radius: 12px; color: #fff; outline: none; transition: all 0.2s;
  }
  .input-field:focus {
  ...
  .toggle-btn {
    ...
    border-radius: 12px; color: var(--txt); cursor: pointer; transition: all 0.2s;
  }
  ...
  .time-row { display: flex; align-items: center; justify-content: center; gap: 14px; }
  .time-btn {
    width: 36px; height: 36px; border-radius: 10px; border: 2px solid var(--glass-border);
    background: var(--glass); color: #fff; font-size: 18px; font-weight: 700;
    cursor: pointer; display: flex; align-items: center; justify-content: center;
    transition: all 0.2s; font-family: 'Fredoka', sans-serif;
  }
  .time-btn:hover { border-color: var(--cyan); background: rgba(0, 229, 255, 0.12); }
  .time-display {
    font-family: 'Fredoka', sans-serif; font-size: 22px; font-weight: 700;
    color: var(--orange); min-width: 50px; text-align: center;
  }

  .start-actions { margin-top: 20px; }
  .hint {
    font-size: 10px; opacity: 0.6; margin-top: 10px; letter-spacing: 1px;
    color: var(--orange); transition: all 0.2s;
  }
  .hint.ready { color: var(--txt); opacity: 0.3; }

  .divider {
    border-top: 1px solid var(--glass-border);
    margin: 18px auto 14px; max-width: 380px;
  }

  .scores-section { margin: 0 auto; max-width: 380px; }
  .no-scores {
    font-size: 12px; opacity: 0.4; padding: 12px; text-align: center;
  }
  .score-list { margin-bottom: 10px; }
  .score-row {
    display: flex; align-items: center; padding: 8px 12px; gap: 10px;
    border-radius: 10px;
  }
  .score-row.odd { background: rgba(255, 255, 255, 0.03); }
  .rank {
    font-family: 'Fredoka', sans-serif; font-size: 14px; font-weight: 700;
    width: 26px; text-align: center; flex-shrink: 0;
  }
  .score-info { flex: 1; text-align: left; min-width: 0; }
  .score-name {
    font-family: 'Fredoka', sans-serif; font-size: 13px; font-weight: 600;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  }
  .score-meta {
    font-family: 'Nunito', sans-serif; font-size: 10px; opacity: 0.45;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  }
  .score-val {
...
.name-chip {
  padding: 4px 12px; font-family: 'Fredoka', sans-serif;
