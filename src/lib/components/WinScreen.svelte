  ...
  {#if game.winData}
    <div class="card wide">
      ...
      <div class="word-review">
        {#each game.winData.words as word}
          <div class="word-entry">
            ...
            <div class="we-pts">
              +{word.p}{#if word.sb > 0} <span class="speed-bonus">⚡{word.sb}</span>{/if}
            </div>
          </div>
        {/each}
      </div>
      ...
      <div class="stats">
        <div class="stat"><div class="stat-value">{game.winData.words.length}</div><div class="stat-label">Words</div></div>
        <div class="stat"><div class="stat-value">{game.winData.totalScore.toLocaleString()}</div><div class="stat-label">Score</div></div>
        <div class="stat"><div class="stat-value">⚡{game.winData.totalSpeedBonus}</div><div class="stat-label">Speed Bonus</div></div>
      </div>
...
<style>
  ...
  .word-review {
    display: grid; gap: 7px; margin: 12px 0 18px; text-align: left;
    max-height: 34vh; overflow-y: auto; padding-right: 6px;
  }
  .word-review::-webkit-scrollbar { width: 6px; }
  .word-review::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.2); border-radius: 3px; }
  .word-entry {
    padding: 9px 12px; background: rgba(255, 255, 255, 0.04);
    border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 10px;
    display: flex; align-items: flex-start; gap: 9px;
  }
  .we-icon { font-size: 18px; flex-shrink: 0; margin-top: 2px; }
  .we-text { flex: 1; }
  .we-word { font-family: 'Fredoka', sans-serif; font-size: 14px; font-weight: 600; color: var(--green); margin-bottom: 1px; }
  .we-def { font-size: 11px; opacity: 0.55; line-height: 1.4; }
  .we-pts { font-family: 'Fredoka', sans-serif; font-size: 11px; color: var(--gold); opacity: 0.7; flex-shrink: 0; align-self: center; }
  .speed-bonus { color: var(--cyan); }
